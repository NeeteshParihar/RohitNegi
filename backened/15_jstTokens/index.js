...
app.post("/login" , validateEmail , async(req , res)=>{
    ...
    try{
        ...
        // sending the tokens or jwt    
        // first "token" is the key and second value is value , generated by JWT or other token types 

        const jwtToken = jwt.sign({emailId , '_id': user._id } , 'neetesh' , {expiresIn: 15});  // expires in 10sec // pasyload and private key or secret key 
        // the generated token is a string having parts : 1.header , 2.payload and 3.digital signature 
...
app.get("/info/:id", async (req, res) => {
    ...
    try {
       ...
       const payload = jwt.verify(req.cookies.jwtToken , 'neetesh');
       ...
       console.log(payload._id)
        ...
        const engineer = await Engineers.findById(payload._id);
...
app.get("/info", async (req, res) => {
    ...
    try {
        ...
        const payload = jwt.verify(req.cookies.jwtToken , 'neetesh'); // for verifed user it gives payload 

        console.log("payload");
          // it has iat which tell the creation time const { jwtToken:{_id} } = req.cookies;
